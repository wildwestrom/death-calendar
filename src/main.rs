...
fn render_svg(bday: Date, years: i16) -> Document {
    ...
    let inner_square_size = 100;
    let padding = 20;
    let outer_square_size = inner_square_size + padding;

    let border_size = 2;
    let grid_width = outer_square_size * years;
    let grid_height = outer_square_size * weeks_in_year;
    let viewbox_width = years * outer_square_size + outer_square_size * border_size;
    let viewbox_height = weeks_in_year * outer_square_size + outer_square_size * border_size;
    ...
    while curr_date < end {
        ...
        let x = ((count / weeks_in_year) * outer_square_size) + x_offset;
        let y_offset = (viewbox_height - grid_height) / 2;
        let y = ((count % weeks_in_year) * outer_square_size) + y_offset;
